...
//
//! The RTOS is a static class, instatiation is not needed.
//! After creating the tasks, call RTOS::run() to start the scheduling
//! the tasks. RTOS::run() will never return.
//
//***************************************************************************
...
private:
   ...
   static void do_statistics_clear (void);
     ...
     //! this is a static class, prevent instatiation
   RTOS( void ){}
   ...
   static void add( channel_base * c );
   static void add( mailbox_base * m );
   static void add( pool_base *p );
#endif
